...
def initiate_polygon_zones(polygons, triggering_anchors=[sv.Position.CENTER]):
    return [sv.PolygonZone(polygon=polygon, triggering_anchors=triggering_anchors) for polygon in polygons]
...
class VideoProcessor:
    def __init__(
        ...
        target_video_path: Optional[str] = "result.mov",
    ...
    def annotate_frame(self, frame, detections):
        ...
        for zone_out_id, zone_out in enumerate(self.zones_out):
            center = sv.get_polygon_center(zone_out.polygon)
            if zone_out_id in self.detections_manager.counts:
                for i, (zone_in_id, track_ids) in enumerate(self.detections_manager.counts[zone_out_id].items()):
                    count = len(track_ids)
                    text_anchor = sv.Point(x=center.x, y=center.y + 40 * i)
...
def run_traffic_analysis(
    source_video_path: str,
    source_weights_path: str = "traffic_analysis.pt",
    target_video_path: Optional[str] = "result.mov",
    confidence_threshold: float = 0.3,
    iou_threshold: float = 0.7,
):
